...
parser.add_argument("--fps", type=int, help="(dataset creation) frames per second, default 25", default=25)
parser.add_argument(
    "--starting-frame", type=int, help="(dataset creation) starting frame, default 1", default=1
)
parser.add_argument(
    "--frame-interval", type=int, help="(dataset creation) space between frames, default 0", default=0
)
...
subprocess.run("export PATH=/koko/system/anaconda/envs/python38/bin:$PATH", shell=True)
...
try:
    ...
    if contains_h264 and contains_mp4:
        ...
        logging.info("Converting .h264 to .mp4, old h264 files can be found in the h264_files folder")
        subprocess.run(["python", "Video_Frame_Counter/h264tomp4.py", ">>", "conversion.log", "2>&1"], shell=True)
    elif contains_mp4:
        logging.info("No conversion needed, making counts.csv")
        subprocess.run("python Video_Frame_Counter/make_counts.py >> conversion.log 2>&1", shell=True)
    else:
...
subprocess.run(f"git clone https://github.com/Elias2660/VideoSamplerRewrite.git >> clones.log 2>&1", shell=True)
subprocess.run(f"python VideoSamplerRewrite/Dataprep.py --frames-per-sample {args.frames_per_sample} --number-of-samples {args.number_of_samples} --normalize {args.normalize} --out-channels {args.out_channels} --max-workers {args.max_workers_video_sampling} >> dataprep.log 2>&1", shell=True)
